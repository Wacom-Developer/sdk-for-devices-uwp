...
## Introduction to CDL - Windows UWP
...
In order to find the available Wacom Ink Devices create a new instance of an ```InkDeviceWatcher``` for each class of device - USB, Bluetooth or Bluetooth LE - you wish to support. 
* **Note:** Bluetooth devices should be in pairing mode in order to be found. To enter pairing mode on a Wacom SmartPad press and hold the button on the device for 6 seconds.*

For example, to watch for USB Ink Devives, create an ```InkDeviceWatcherUSB``` : 
